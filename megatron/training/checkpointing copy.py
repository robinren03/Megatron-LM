...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, load_arg='load', strict=True,
    ...
    from convert_dict import convert_state_dict
    ...
    if ckpt_type == CheckpointType.LEGACY and args.ckpt_format == "torch_dcp":
        dtensor_state_dict = _to_dtensor(ddp_model, state_dict)
        state_dict = dtensor_state_dict
    ...
    if not skip_load_to_model_and_opt:
        if len(ddp_model) == 1:
            ddp_model[0].load_state_dict(state_dict, strict=strict)
        else:
